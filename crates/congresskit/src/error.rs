//! Unified error type for congresskit. All public methods return `congresskit::Result<T>`.
use thiserror::Error;
...
mod tests {
    ...
    fn checksum_mismatch_displays_both_digests() {
        let e = Error::ChecksumMismatch {
            file: "congress-2024.parquet".into(),
            expected: "aaa".into(),
            actual: "bbb".into(),
        };
        let s = e.to_string();
        assert!(s.contains("aaa") && s.contains("bbb") && s.contains("congress-2024.parquet"));
    }
}
